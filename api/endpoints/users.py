...
@router.post("/users")
async def create_user(user_data: Annotated[models.UserCreate, Depends(models.UserCreate.create_form)], current_user: Annotated[models.User, Depends(security.check_roles(["admin"]))]):
  return users.create_user(user_data)

@router.put("/users/{username}")
async def update_user(user_data: Annotated[models.UserCreate, Depends(models.UserUpdate.update_form)], username: str, current_user: Annotated[models.User, Depends(security.check_roles(["admin"]))]):
